...
// Shared price breakdown — no item list and NO total row (the COD/Collect block
// below already shows the total, so listing it here too just duplicates it).
  ...
  if (order.courier?.consignmentId) {
    return (
      <div className="text-center leading-none">
        <p className={`uppercase text-black ${big ? "text-[9px]" : "text-[6px]"}`}>Consignment No.</p>
...
// `label-ink` forces every tint to solid #000 in print/preview/PDF (see CSS) —
// thermal heads render gray/opacity as patchy ash, so we keep ink pure black.
function Label({ order, shop, opts, layout, compact = false }) {
  ...
  if (compact && layout !== "landscape") {
    return (
      <div className="label-content label-ink text-black bg-white flex flex-col" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
        {/* Header */}
        <div className="flex items-start justify-between gap-[1mm] border-b border-black pb-[0.8mm]">
          <div className="min-w-0">
            ...
            {shop.merchantId && <p className="text-[9px] font-semibold leading-tight truncate">{shop.merchantId}</p>}
          </div>
        ...
        {/* Deliver to */}
        <div className="py-[1mm] border-b border-black">
          <p className="text-[6px] uppercase text-black leading-none">Deliver to</p>
  ...
  if (compact && layout === "landscape") {
    return (
      <div className="label-content label-content--landscape label-ink text-black bg-white" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
        <div className="grid grid-cols-[1fr_34mm] gap-[2mm] h-full">
          ...
          <div className="min-w-0 flex flex-col">
            <div className="border-b border-black pb-[1mm] min-w-0">
              ...
              {shop.merchantId && <p className="text-[8px] font-semibold leading-tight truncate">{shop.merchantId}</p>}
            </div>

            <div className="py-[1mm] min-w-0">
              <p className="text-[5.5px] uppercase text-black leading-none">Deliver to</p>
          ...
          <div className="min-w-0 flex flex-col border-l border-black pl-[2mm]">
            {opts.showBarcode && (
  ...
  return (
    <div className="label-content label-content--portrait label-ink text-black bg-white" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
      {/* Header */}
      <div className="flex items-start justify-between gap-2 border-b-2 border-black pb-1.5">
        <div className="min-w-0">
          ...
          {shop.merchantId && <p className="text-[12px] font-semibold leading-tight">{shop.merchantId}</p>}
        </div>
      ...
      {/* QR code — scannable even if the thermal print is partly smudged. */}
      {opts.showBarcode && (
        <div className="py-2 flex flex-col items-center border-b border-black">
      ...
      <div className="py-2 border-b border-black">
        <p className="text-[9px] uppercase tracking-wide text-black">Deliver to</p>
...
export default function LabelsPage() {
  ...
  const shop = {
    ...
    merchantId: settings?.siteInfo?.merchantId || "",
  };
  ...
  const [sizeKey, setSizeKey] = useState("2x3");
  ...
  // Persist print settings so they survive refreshes / sessions on each PC.
  useEffect(() => {
    try {
      const s = JSON.parse(localStorage.getItem("labelPrintSettings") || "{}");
      if (s.sizeKey) setSizeKey(s.sizeKey);
      if (s.marginMm !== undefined) setMarginMm(String(s.marginMm));
      if (s.scalePct !== undefined) setScalePct(String(s.scalePct));
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "labelPrintSettings",
        JSON.stringify({ sizeKey, marginMm, scalePct, customW, customH, opts })
      );
    } catch {}
  }, [sizeKey, marginMm, scalePct, customW, customH, opts]);
  ...
  const pad = size.pad;
  // M = the physical label (what the printer driver is set to).
  const Mw = sizeKey === "custom" ? safeDim(customW, "100mm") : size.w;
  const Mh = sizeKey === "custom" ? safeDim(customH, "150mm") : size.h;
  const labelLayout = parseFloat(Mw) >= parseFloat(Mh) ? "landscape" : "portrait";
  ...
  // The @page is the EXACT physical label and margin + scale are baked in, so
  // the operator never touches Chrome's Margins/Scale options. The design prints
  // upright as-is (no rotation) — match the Label size to your stock.
  ...
  // so the preview is WYSIWYG — same page shape & scale as print.
  ...
  const printCss = `
    ...
    /* Force pure black ink everywhere the label renders (print / preview / PDF).
       Thermal heads turn any gray or opacity tint into patchy ash, so collapse
       every text + border tint to solid #000. */
    .label-ink, .label-ink * { color: #000 !important; border-color: #000 !important; }
  ...
  // printer stops re-fitting/scaling the way the browser print dialog can.
  const generatePdf = async (mode) => {
    ...
    try {
      ...
      for (let i = 0; i < selectedOrders.length; i++) {
        ...
        // Compose onto a physical-page canvas with the chosen scale baked in.
    ...
    <div>
        ...
        <div className="space-y-4">
          <Card className="space-y-3">
            ...
            <p className="text-[11px] text-brand-tan leading-relaxed">
              ...
              <b> Margins: Default</b> and <b>Scale: 100%</b> (don&apos;t change them there). Just pick the
              <b> label size that matches your stock</b>. <b>Print</b> (browser) is fastest for bulk; PDF is the fallback.
            </p>
          ...
          <Card>
            ...
            <div className="bg-brand-cream/40 rounded-lg p-3 overflow-auto flex justify-center" style={{ maxHeight: "55vh" }}>
              {selectedOrders[0] ? (
                ...
                // to fit; the inner box is scaled exactly like print. If content
                // spills past the dashed edge, the Label size doesn't match stock.
            ...
            <p className="text-[11px] text-brand-tan mt-2">
              Exactly as it prints · <b>{Mw}×{Mh}</b> · {selectedOrders.length || 0} selected.
      ...
      <div aria-hidden className="no-print" style={{ position: "fixed", left: "-99999px", top: 0, pointerEvents: "none" }}>
        {selectedOrders.map((o) => (
          <div key={o._id} id={`cap-${o._id}`} style={{ width: Mw, height: Mh, padding: pad, boxSizing: "border-box", background: "#fff" }}>
