...
export function orderConfirmationTemplate(order) {
  const items = order.items.map((item) =>
    `<tr>
      ...
      <td style="padding:8px 0;border-bottom:1px solid #f0ebe2;color:#2C1810;font-size:14px;text-align:right;">Tk ${(item.price * item.quantity).toLocaleString()}</td>
    </tr>`
  ).join("");
  return base(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">${items}
      <tr><td style="padding:12px 0;color:#2C1810;font-weight:600;">Total</td>
        <td style="padding:12px 0;color:#B85C3A;font-weight:700;font-size:16px;text-align:right;">Tk ${order.totalAmount.toLocaleString()}</td></tr>
    </table>
...
export function orderStatusTemplate(order) {
  const statusMessages = {
    ...
    return_requested: "We've been told a return is on its way back to us. We'll confirm as soon as it arrives and has been checked.",
    partial_returned: "We've received part of your order back. The returned items have been taken off your order total.",
    returned: "We've received your return. Your order total has been adjusted for the items that came back.",
  ...
  return base(`
    ...
    <div style="background:#F5F0E8;border:1px solid #ddd5c8;padding:16px 20px;display:inline-block;">
      ...
      <p style="margin:4px 0 0;color:#2C1810;font-size:18px;font-weight:600;">${statusLabel}</p>
    </div>
  `);
}
