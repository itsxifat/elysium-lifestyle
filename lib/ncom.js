...
//   NCOM_ORDER_KEY_ID       key id ncom sends when handing us an order
//   NCOM_ORDER_SECRET       HMAC secret ncom signs order handoffs with
...
// Where ncom posts orders it wants us to process. Pasted into their
// Settings → Order handling, after the site's own origin.
//
// A full path rather than a base, because that is what their field asks for:
// merchants host this on everything from a clean route to a query string on an
// admin-ajax endpoint, and a platform guessing a suffix is a 404 nobody can
// explain.
export const ORDER_INTAKE_PATH = "/api/ncom/orders";
...
export async function getNcomConfig({ fresh = false } = {}) {
  ...
  const cfg = {
    ...
    // Inbound orders. Issued by ncom on Settings → Order handling. Gated by
    // its own switch, so a shop can hold credentials without yet being live.
    orderKeyId: env("NCOM_ORDER_KEY_ID") || (db.orderKeyId || "").trim(),
    orderSecret: env("NCOM_ORDER_SECRET") || (db.orderSecret || "").trim(),
    acceptOrders: Boolean(db.acceptOrders),
...
// True when ncom may hand us orders right now.
//
// Deliberately independent of `enabled`. That switch governs whether we SERVE
// our catalogue; this one governs whether we ACCEPT orders, and a shop can
// reasonably do the second without the first — ncom may be selling products it
// stores itself on a page that is ours in name only.
export async function orderIntakeLive() {
  const cfg = await getNcomConfig();
  return Boolean(cfg.acceptOrders && cfg.orderSecret && cfg.orderKeyId);
}
