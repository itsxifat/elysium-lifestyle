...
// The variants toNcomProduct will drop because they collide on the option
// combination. Kept in step with the filter above so the migrate can name what
// it silently lost.
export function duplicateVariants(product) {
  const all = product.variants || [];
  const useOption = all.some((v) => isMeaningfulSize(v.size));
  const seen = new Set();
  const dupes = [];
  for (const v of all) {
    const key = useOption && isMeaningfulSize(v.size) ? String(v.size).trim() : "__single__";
...
export function toNcomProduct(product, categoryMap = new Map()) {
  const all = (product.variants || []).map((v) =>
  ...
  // Two variants that map to the same option combination are rejected — and
  // ncom fails the WHOLE product for it, so one duplicated size costs the
  // product. Our schema allows it (a duplicated size row is a data-entry slip),
  // so collapse to the first of each and let the caller report what it dropped.
  const seen = new Set();
