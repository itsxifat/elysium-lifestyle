...
// with plain node, which has no "@/" alias resolution.
import { sizeCode } from "./sku.js";

// ncom.bd integration — catalogue push + two-way stock sync.
//
// Ships INERT: without NCOM_API_KEY every helper here is a no-op, so the
// storefront behaves exactly as it does today until the key is added on the VPS.
// Same pattern as the tracking + courier integrations.
//
...
//   NCOM_API_URL         override for the base URL (defaults to production)
//
...
export const NCOM_API = process.env.NCOM_API_URL || "https://ncom.bd/api/v1";

export function ncomKey() {
  return (process.env.NCOM_API_KEY || "").trim();
}

export function ncomEnabled() {
  return Boolean(ncomKey());
}
...
export async function ncomFetch(path, { method = "GET", body, retries = 3, key } = {}) {
  const apiKey = key || ncomKey();
  if (!apiKey) throw new NcomError("unauthorized", "NCOM_API_KEY is not set");

  let attempt = 0;
  for (;;) {
    let response;
    try {
      response = await fetch(NCOM_API + path, {
        method,
        headers: {
...
export async function ncomList(path, { limit = 250, key } = {}) {
  const out = [];
  for (let page = 1; ; page++) {
    const joiner = path.includes("?") ? "&" : "?";
    const { data, pagination } = await ncomFetch(`${path}${joiner}page=${page}&limit=${limit}`, { key });
    out.push(...(data || []));
...
export function reportStockDelta(lines, { reason = "MANUAL", note = "" } = {}) {
  if (!ncomEnabled()) return Promise.resolve(null);
