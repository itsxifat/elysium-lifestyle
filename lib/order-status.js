...
/**
 * Statuses where a courier's answer can no longer change anything.
 *
 * `cancelled` because a cancellation WE made is a decision, never overridden by
 * a lagging courier record (see shouldApply in lib/courier-sync) — and in
 * practice Steadfast answers 401 for a consignment we cancelled on their side,
 * so asking produced nothing but a wall of identical errors.
 *
 * The two computed return states because they rank above everything a courier
 * can report: once staff have itemised what came back, no delivery status can
 * move the order.
 *
 * So a sync does not look these up at all — not even when asked to include
 * parcels already settled at Steadfast, which relaxes THEIR side of the filter,
 * never ours.
 */
export const COURIER_FINAL_STATUSES = ["cancelled", "partial_returned", "returned"];

export function isCourierFinal(status) {
  return COURIER_FINAL_STATUSES.includes(status);
}
