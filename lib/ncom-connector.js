...
export async function reserveUnits(orderRef, lines) {
  ...
  const existing = await NcomReservation.findOneAndUpdate(
    ...
    // returnDocument:"before" — i.e. null when this call is the one that
    // inserted, which is exactly how the claim is detected. (`new: false` said
    // the same thing and is deprecated in mongoose 9.)
