...
import {
  ...
  getSteadfastConfig,
  steadfastConfigured,
  verifyCourierCredentials,
} from "@/lib/steadfast";
import { courierStatusLabel, mapSteadfastStatus, COURIER_SETTLED_STATUSES } from "@/lib/steadfast-status";
import { COURIER_FINAL_STATUSES, isReturnStatus, orderStatusLabel } from "@/lib/order-status";
...
export async function syncOneOrder(order, cfg, { actorName = "Steadfast sync" } = {}) {
  try {
    ...
    order.courier.lastSyncedAt = new Date();
    order.courier.error = "";
    // They answered, so whatever refused this parcel before is over.
    order.courier.unauthorizedAt = null;
    const applied = await applyCourierStatus(order, raw, { actorName });
    return { ok: true, ...applied };
  } catch (err) {
    // The credentials were checked before the run started, so a 401 here is
    // Steadfast refusing THIS consignment — not our keys. It is a permanent
    // answer, so the parcel is marked and dropped out of future runs instead of
    // producing the same failure line for ever.
    const unauthorized = !!err?.auth;
    const message = unauthorized
      ? "Not accessible on this Steadfast account (401 Unauthorized Access) — no longer checked automatically."
      : err?.message || "Lookup failed";
    try {
      order.courier.lastSyncedAt = new Date();
      order.courier.error = message;
      if (unauthorized) order.courier.unauthorizedAt = new Date();
      await order.save();
    } catch {
...
async function applyReturnRequests(cfg, { orderIds, actorName } = {}) {
  ...
  // Same rule as the status pass: an order we cancelled, or whose return is
  // already itemised, is not moved by anything they report.
  const query = { $or: or, orderStatus: { $nin: COURIER_FINAL_STATUSES } };
...
export async function syncCourierStatuses({
  ...
  // One call to settle the question a parcel's 401 cannot answer: are the keys
  // good? Everything downstream depends on knowing this for certain — with a
  // valid key, a 401 on a consignment means that consignment, and the run
  // carries on past it.
  const credentials = await verifyCourierCredentials(cfg);
  if (!credentials.ok) {
    return {
      ok: false,
      authFailed: credentials.auth,
      error: credentials.auth
        ? `Steadfast refused the API credentials (${credentials.error}). Nothing was checked — fix the Api-Key and Secret-Key on the Courier page.`
        : credentials.error,
    };
  }
  ...
  const query = {
    ...
    // Orders whose fate is already decided here are never looked up: a courier
    // answer could not change them, and a consignment we cancelled on their
    // side answers 401, which is where a wall of identical errors came from.
    // Deliberately outside the `includeSettled` branch below — that flag
    // relaxes THEIR side of the filter, never ours.
    orderStatus: { $nin: COURIER_FINAL_STATUSES },
  };
  if (Array.isArray(orderIds) && orderIds.length) {
    query._id = { $in: orderIds };
  } else {
    // Consignments Steadfast has already refused by name are left out of bulk
    // runs. Naming specific orders overrides that — asking about one parcel is
    // a deliberate act, and if they answer, the flag is cleared.
    query["courier.unauthorizedAt"] = null;
  }
  if (!includeSettled) query["courier.status"] = { $nin: COURIER_SETTLED_STATUSES };

  const eligible = await Order.countDocuments(query);

  // Those same orders may still be carrying the error from the last time they
  // WERE looked up, which now reads as an unresolved problem on an order
  // nobody is going to check again. Cleared once per run.
  const cleared = await Order.updateMany(
    {
      orderStatus: { $in: COURIER_FINAL_STATUSES },
      "courier.error": /Steadfast API error/i,
    },
    { $set: { "courier.error": "" } }
  );
  ...
  // that can spot a return raised after a successful delivery.
  const returnPass = await applyReturnRequests(cfg, { orderIds, actorName });
  ...
  return {
    ok: true,
    // Reaching here means the credentials were verified up front, so the run
    // never "fails" over a parcel — a refused consignment is one line in the
    // report and one order dropped from future runs.
    authFailed: false,
    checked: results.length + returnOnly,
    updated: changes.length,
    unchanged: results.filter((r) => r.ok && !r.statusChanged && !movedIds.has(r.orderId)).length,
    failed: errors.length,
    // Of those failures, how many were Steadfast refusing the consignment
    // itself. Counted separately because it is not a fault to retry: those
    // orders are now out of the bulk run for good.
    unauthorized: errors.filter((e) => e.unauthorized).length,
    ...
    includeSettled,
    settledSkipped,
    // Stale "(401)" errors wiped off orders that are no longer looked up.
    clearedErrors: cleared?.modifiedCount || 0,
