...
const FB_COOKIE_DAYS = 90; // Meta's own lifetime for _fbp/_fbc.
...
/**
 * Guarantee _fbc (the ad click id) and _fbp (a stable browser id) exist as
 * first-party cookies, in Meta's documented format.
 *
 * The pixel writes these itself — but only if it loads. Ad traffic is exactly
 * the traffic most likely to block it, and a blocked pixel means CAPI matches
 * on name/phone alone and the fbclid the ad appended is lost on the first
 * navigation. Writing them ourselves *before* injecting the pixel means the
 * pixel adopts what it finds, so there is one id either way and no second
 * conflicting cookie.
 *
 * Returns the ids so callers don't have to re-read the cookies.
 */
export function ensureClickIds() {
...
export async function initTracking() {
  ...
  initialized = true;
  // Before anything can block or fail: capture the click id off the URL and
  // pin a browser id. The pixel injected below reuses both.
  ensureClickIds();
