...
export async function runFraudCheckForOrder(orderId, phone) {
  try {
    ...
    // Normalise here too so rechecks of older orders (whose stored phone may be
    // in Bangla digits) hit the courier API with a clean ASCII number.
    ...
    const minSuccessRate = cfg.minSuccessRate ?? 0; // 0 = ignore
    const maxFrauds = cfg.maxFrauds ?? 0; // block auto-process above this fraud count
    const meetsThreshold =
      totalParcels >= minDelivery &&
      delivered >= minSuccessful &&
