...
/**
 * Are the saved credentials actually good?
 *
 * Asked once at the start of a sync, because a 401 on a single parcel does NOT
 * mean the keys are wrong — Steadfast answers `401 Unauthorized Access` for a
 * consignment that is no longer accessible on the account, and we have parcels
 * from last August that do exactly that while today's parcels answer 200 with
 * the same keys. Inferring "credentials refused" from one parcel aborted whole
 * runs over three dead consignments.
 *
 * /get_balance is the cheapest call that needs nothing but the credentials, so
 * its answer separates the two cases for certain rather than by guesswork.
 */
export async function verifyCourierCredentials(cfg) {
  try {
    await callSteadfast(await resolveCfg(cfg), "/get_balance");
    return { ok: true };
  } catch (err) {
    return { ok: false, auth: !!err?.auth, error: err?.message || "Steadfast could not be reached" };
  }
}
