...
const SUMMARY_FIELDS =
  "state trigger scope includeSettled by byName startedAt finishedAt progress totals error authFailed returnRequestError reconstructed note";
...
export async function executeRun(runId, { orderIds, includeSettled = false, actorName } = {}) {
  try {
    ...
    if (!report.ok) {
      await finish(runId, { state: "failed", error: report.error, authFailed: !!report.authFailed });
      return report;
    }
    ...
    await finish(runId, {
      ...
      totals: {
        ...
        unauthorized: report.unauthorized || 0,
      },
    });
    return report;
  } catch (err) {
