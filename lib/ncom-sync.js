...
import {
  ncomFetch, ncomList, toNcomProduct, getNcomConfig, resolveImageUrls,
  duplicateVariants, PRICE_CURRENCY,
} from "./ncom.js";

// SKUs owned by more than one variant. Stock sync addresses variants BY SKU, so
// a duplicate is not cosmetic: a push meant for one variant lands on whichever
// the lookup happens to return, and the reconcile compares against whichever
// won the map. Both sides then disagree forever, in a way that looks like drift.
...
export async function syncStatus() {
  ...
  const local = {
    ...
    missingSku,
    // Two variants sharing a SKU cannot both sync — surfaced as a stat so it
    // does not have to be inferred from a confusing reconcile report.
    duplicateSkus: duplicateSkus(products).length,
...
export async function migrateCatalogue({ dryRun = true, skipStock = false } = {}) {
  ...
  for (const cat of ordered) {
    ...
    // Create fresh, and remember the id locally.
    const create = async () => {
      const { data } = await ncomFetch("/categories", { method: "POST", body });
      await Category.updateOne({ _id: cat._id }, { $set: { ncomId: data.id } });
      return data.id;
    };
    ...
    try {
      ...
      if (remoteId) {
        try {
          ...
          // The stored id points at nothing — the category was deleted there,
          // or (much more likely) the API key now points at a DIFFERENT
          // workspace than the one these ids came from. Re-create and re-link
          // rather than failing: a stale pointer must not permanently break the
          // category, because an empty category map silently imports every
          // product uncategorised.
          if (e.code !== "not_found") throw e;
          remoteId = await create();
          staleLinks++;
          log.info(`  re-created ${cat.name} (previous link was stale)`);
        }
      } else {
        remoteId = await create();
  ...
  if (staleLinks) {
    log.warn(
      `${staleLinks} categor(ies) had a stored link to a category that no longer exists — they were ` +
      `re-created here. That normally means the API key now points at a different workspace than the ` +
      `one they were first imported into.`
    );
  }

  // Products are filed by category id, so an empty map means every product
  // imports uncategorised. Worth stopping for rather than discovering later.
  if (categories.length && categoryMap.size === 0) {
    log.error("Every category failed, so all products would import with no category. Stopping — nothing was pushed.");
    return { ok: false, log: log.lines, stats };
  }
  ...
  // Duplicate SKUs across the catalogue — stock sync keys on SKU, so these
  // cannot be reconciled correctly no matter what either side does.
  const dupSkus = duplicateSkus(products);
  if (dupSkus.length) {
    const affected = dupSkus.reduce((n, [, list]) => n + list.length, 0);
    log.error(
      `${dupSkus.length} SKU(s) are used by ${affected} different variants. Stock is addressed by SKU, ` +
      `so those variants cannot sync correctly — a push for one lands on another, and the nightly ` +
      `reconcile will keep reporting drift that never settles.`
    );
    for (const [sku, list] of dupSkus.slice(0, 10)) log.error(`  ${sku} → ${list.join(" | ")}`);
    if (dupSkus.length > 10) log.error(`  …and ${dupSkus.length - 10} more`);
    log.warn("Fix by re-running the SKU backfill after clearing the duplicates, or editing the products.");
  }

  // Variants that collide on their option combination get dropped rather than
  // failing the whole product — but say which, because it is lost stock.
  const withDupVariants = products
    .map((p) => ({ p, dupes: duplicateVariants(p) }))
    .filter((x) => x.dupes.length);
  if (withDupVariants.length) {
    log.warn(
      `${withDupVariants.length} product(s) have two variants with the same size. ncom allows only one ` +
      `variant per option combination, so the extra is dropped — otherwise it rejects the whole product.`
    );
    for (const { p, dupes } of withDupVariants.slice(0, 10)) {
      log.warn(`  ${p.name} — dropping duplicate ${dupes.map((d) => `${d.size}${d.sku ? ` (${d.sku})` : ""}`).join(", ")}`);
    }
    if (withDupVariants.length > 10) log.warn(`  …and ${withDupVariants.length - 10} more`);
  }
...
export async function reconcileStock({ dryRun = true } = {}) {
  ...
  for (const p of products) {
    for (const v of p.variants || []) {
      if (!v.sku) continue;
      variantCount++;
      ours.set(v.sku, { stock: Math.max(0, Number(v.stock) || 0), product: p.name, size: v.size });
    }
  }

  // The map is keyed by SKU, so duplicates collapse and only the last one's
  // stock is ever compared. Without this note the counts look like a bug here.
  if (variantCount > ours.size) {
    log.warn(
      `${variantCount} variants carry only ${ours.size} distinct SKUs — ${variantCount - ours.size} are ` +
      `duplicates. Only one variant per SKU can be reconciled, so some of the drift below will keep ` +
      `coming back until the SKUs are made unique.`
    );
  }
  ...
  for (const [sku, local] of ours) {
    ...
    // Only the rows that actually disagree are pushed.
    //
    // Writing all 838 every night looked harmless — the values match, so the
    // counts do not move — but ncom emits an inventory.updated webhook per
    // accepted row and every one of those comes straight back at us. That storm
    // is what overran their 10s delivery timeout and got the endpoint parked
    // after 20 consecutive failures. A reconcile that agrees should be silent.
  ...
  if (!updates.length) {
    ...
    log.ok("Both sides already agree on every SKU — nothing to push.");
    return { ok: true, log: log.lines, stats: { ...stats, pushed: 0 } };
  }
