...
import { ncomFetch, ncomList, toNcomProduct, getNcomConfig, PRICE_CURRENCY } from "./ncom.js";
...
export async function migrateCatalogue({ dryRun = true, skipStock = false } = {}) {
  ...
  // of it buries the single fact you need, so identical errors are counted and
  // reported once, with a couple of example ids.
  const errorTally = new Map();
  const tally = (message, externalId) => {
    const entry = errorTally.get(message) || { count: 0, examples: [] };
    entry.count++;
    if (entry.examples.length < 3) entry.examples.push(externalId);
    errorTally.set(message, entry);
  };
  ...
  for (let i = 0; i < products.length; i += IMPORT_BATCH) {
    ...
    try {
      ...
      for (const problem of data.errors || []) tally(problem.error, problem.externalId);
    } catch (e) {
