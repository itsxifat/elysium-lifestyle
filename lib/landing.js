...
// Shape an offer for the public page: enough to render prices and size pickers,
// nothing more.
export async function publicOffers(page) {
  ...
  for (const offer of active) {
    ...
    for (const line of offer.items) {
      ...
      if (!p || p.isPublished === false || !p.variants?.length) {
        missing = true;
        break;
      }
      const quantity = Math.max(1, Number(line.quantity) || 1);
      ...
      // Only sizes that are actually in stock can be picked. A pinned line still
      // publishes its one size+price: the page needs the unit price to re-derive
      // the total when ANOTHER line's size changes.
      const inStock = p.variants.filter((v) => v.stock >= quantity);
      const sizes = (pinned ? inStock.filter((v) => v.size === pinned) : inStock).map((v) => ({
        size: v.size,
        price: v.price,
      }));
      if (!sizes.length) {
        missing = true; // pinned size sold out, or nothing left in any size
        break;
      }

      // The page pre-selects the first in-stock size, so preview that one. The
      // client re-derives the total (same rule, see lib/landing-pricing) as soon
      // as the shopper switches size.
    ...
    if (missing) continue; // hide sold-out / unpublished offers entirely
    ...
    out.push({
      ...
      items: lines,
      // The discount rule, so the page can re-price live on a size change.
      pricing: { mode: offer.pricingMode || "auto", value: Number(offer.priceValue) || 0 },
      // A manual compareAtPrice is absolute; otherwise strike through the
      // regular total (and only when the offer actually saves money).
