...
function buildAlacarteOffer(offer, byId) {
  ...
  // The least the customer could actually leave with: the minimum quantity of the
  // cheapest item, with the offer's flat discount applied to *that* cart. Not
  // shown on the page (the pool prices each item itself) — it's the headline
  // value for ViewContent, so it has to be a total someone could really pay.
  // Discounting a single item by the whole cart's discount, as this used to,
  // priced a bundle below any of its own contents.
  ...
  return {
    ...
    price: applyOfferPricing(offer, cheapest * minQty),
  };
}
...
function buildCollectionOffer(offer, byId) {
  ...
  return {
    ...
    maxQty: max,
    manualCompareAt: Number(offer.compareAtPrice) > 0 ? Number(offer.compareAtPrice) : 0,
    // Headline price = the cheapest rung, i.e. the least a customer could pay.
    // The page shows the ladder itself rather than this one number.
    price: tiers[0].price,
  };
}
